...
class TestCategories:
    ...
    def test_get_single_skill_not_found(self):
        ...
        assert response.json()["detail"] == skills_testdata.GetSingleSkillNotFound.EXPECTED_DETAIL
    ...
    def test_create_duplicate_skill(self):
        ...
        assert response.json()["detail"] == skills_testdata.CreatDuplicateSkill.EXPECTED_DETAIL

    def test_delete_skill_not_found(self):
        response = test_client.delete(skills_testdata.DeleteSkillNotFound.QUERY_PATH)
        assert response.status_code == 404
        assert response.json()["detail"] == skills_testdata.DeleteSkillNotFound.EXPECTED_DETAIL
    ...
    def test_create_skill_check_result(self):
        double_check = test_client.get(skills_testdata.CreateSkillCheckResult.QUERY_PATH)
        assert double_check.status_code == 200
        assert double_check.json() == skills_testdata.CreateSkillCheckResult.EXPECTED_RESULT

    @pytest.mark.dependency()
    def test_delete_skill(self):
        response = test_client.delete(skills_testdata.DeleteSkill.QUERY_PATH)
        assert response.status_code == 200
        assert response.json()["message"] == skills_testdata.DeleteSkill.EXPECTED_MESSAGE
    ...
    def test_delete_skill_check_result(self):
        double_check = test_client.get(skills_testdata.DeleteSkillCheckResult.QUERY_PATH)
        assert double_check.status_code == 404
        assert double_check.json()["detail"] == skills_testdata.DeleteSkillCheckResult.EXPECTED_DETAIL
